  ...
  .handler(async ({ context }): Promise<FullExportResponse> => {
    ...
    try {
      ...
      await supabaseAdmin.from("activity_logs").insert({
        ...
        actor_email: (profile?.email as string | null) ?? null,
        event_type: "exportacao",
        area: "Exportação de dados",
        screen: "Exportar dados",
        route: "/exportacao",
        action: "exportacao_completa",
        entity: "organizations",
        record_id: organizationId,
        summary: `Exportação completa gerada: ${result.fileCount} arquivos, ${result.recordCount} registros, ${result.bytes} bytes.`,
        new_data: {
      ...
      await supabaseAdmin.from("activity_logs").insert({
        ...
        entity: "organizations",
        record_id: organizationId,
        summary: `Falha na exportação completa: ${message}`,
        new_data: { status: "erro", erro: message, duracao_ms: Date.now() - startedAt },
      } as never);
