...
import { REQUEST_LABELS, competenceLabel, useReopenRequests } from "@/lib/transparency";
...
export function ReopenReviewTab() {
  ...
  async function execute(id: string) {
    setBusy(true);
    const { error } = await supabase.rpc("execute_transparency_reopen", { _request: id, _justification: "Reabertura efetivada pelo suporte da plataforma." });
    setBusy(false);
    if (error) {
      toast.error(error.message);
      return;
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Chamados de reabertura de competência de todos os órgãos. Apenas o suporte da plataforma pode aprovar e
        efetivar a reabertura; nenhuma versão publicada é apagada.
      </p>
          ...
          <TableBody>
            ...
            {requests.map((r: any) => (
              <TableRow key={r.id}>
                ...
                <TableCell>{r.organization?.short_name ?? r.organization?.legal_name ?? "—"}</TableCell>
      ...
      <Dialog open={!!current} onOpenChange={(v) => !v && setCurrent(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setCurrent(null)}>Cancelar</Button>
            <Button onClick={review} disabled={busy}>Registrar parecer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
