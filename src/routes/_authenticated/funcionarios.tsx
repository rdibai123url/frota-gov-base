...
import { ROLE_LABELS, supabase, useInvalidate, usePerms, useUnits, type AppRole } from "@/lib/frotagov";
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Funcionários do órgão — FrotaGov" },
      {
        ...
        content: "Pessoas do órgão e funções institucionais reutilizadas em contratos, autorizações e conferências.",
      },
...
const ASSIGNABLE_ROLES: AppRole[] = ["org_admin", "fleet_manager", "unit_manager", "operator", "auditor"];
  ...
  async function onSubmit(ev: React.FormEvent<HTMLFormElement>) {
    ...
    const { error } = editing
      ? await supabase
          ...
          .eq("id", editing.id)
      : await supabase.from("employees").insert({ ...payload, organization_id: orgId!, created_by: userId });
    setSaving(false);
    if (error) {
      toast.error(
        error.code === "23505"
  ...
  return (
    <>
            ...
            {paged.rows.map((e) => (
              <TableRow key={e.id}>
                <TableCell className="font-medium">
                  {e.full_name}
                  {e.job_title && <span className="block text-xs text-muted-foreground">{e.job_title}</span>}
                </TableCell>
                <TableCell className="text-sm">
                  {e.cpf ? maskCPF(e.cpf) : "—"}
                  {e.registration && <span className="block text-xs text-muted-foreground">Mat. {e.registration}</span>}
                </TableCell>
                <TableCell className="text-sm">{unitName(e.unit_id)}</TableCell>
                <TableCell>
                  <div className="flex flex-wrap gap-1">
                    {(e.functions ?? []).length === 0 && <span className="text-sm text-muted-foreground">—</span>}
                    {(e.functions ?? []).map((f) => (
                ...
                <TableCell>
                  <Badge variant={e.active ? "default" : "outline"}>{e.active ? "Ativo" : "Inativo"}</Badge>
                </TableCell>
                <TableCell>
      ...
      <p className="mt-4 text-xs text-muted-foreground">
        Função institucional e perfil de acesso são coisas diferentes: a função descreve o papel da pessoa no órgão
        (fiscal, gestor, autorizador…), enquanto o perfil de acesso define o que o usuário pode fazer no sistema.
      </p>

      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? `Editar ${editing.full_name}` : "Novo funcionário"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={onSubmit} className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <Input id="registration" name="registration" defaultValue={editing?.registration ?? ""} />
              </div>
              <div>
              ...
              <div className="sm:col-span-2">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Selecione todas as funções exercidas. Não confundir com o perfil de acesso do usuário.
                </p>
              </div>
              <div className="sm:col-span-2">
          ...
          {accessFor && (
            <form onSubmit={onCreateAccess} className="space-y-4">
              <p className="text-sm text-muted-foreground">
                O acesso será criado para <strong>{accessFor.full_name}</strong> reaproveitando CPF, telefone, cargo e
                unidade já cadastrados. A pessoa não é duplicada: o funcionário passa a ficar vinculado ao usuário.
              </p>
              <div>
                <Label htmlFor="access_email">E-mail institucional *</Label>
                <Input id="access_email" name="email" type="email" defaultValue={accessFor.email ?? ""} required />
              </div>
              <div>
              ...
              <p className="text-xs text-muted-foreground">
                Uma senha temporária será gerada e exibida uma única vez; a troca é obrigatória no primeiro acesso.
              </p>
              <DialogFooter>
