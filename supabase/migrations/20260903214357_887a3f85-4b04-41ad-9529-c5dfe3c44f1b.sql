CREATE OR REPLACE FUNCTION public.transparency_snapshot(_org uuid, _year integer, _month integer)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
begin
  select jsonb_build_object(
    ...
    'limpeza', (
      select jsonb_build_object('registros', count(*), 'valor_total', coalesce(sum(total_value),0))
      from public.vehicle_cleanings
      where organization_id = _org and status = 'realizada' and performed_at >= ini and performed_at < fim
    ),
    'utilizacao', (
      select jsonb_build_object('registros', count(*))
